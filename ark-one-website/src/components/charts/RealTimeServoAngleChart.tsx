...
const RealTimeServoTowerAngleChart: React.FC<RealTimeChartProps> = ({ esp32_unique_id }) => {
  ...
  const fetchGraphData = useCallback(async () => {
    try {
      ...
      const response = await api.get(
        `Analytics/generateServoTowerAngleTrendGraph?esp32_unique_id=${esp32_unique_id}`,
        { responseType: 'blob' }
      );
