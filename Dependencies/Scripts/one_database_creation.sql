...
CREATE TABLE category (
    ...
    CONSTRAINT fk_parent_category FOREIGN KEY (id_father_category) REFERENCES category(id_category) ON DELETE SET NULL  -- Optional reference to parent category
);
...
CREATE TABLE products (
    ...
    CONSTRAINT fk_product_category FOREIGN KEY (id_category) REFERENCES category(id_category) ON DELETE SET NULL  -- Reference to product category
);
...
CREATE TABLE user_data (
    ...
    registration_date TIMESTAMP WITH TIME ZONE DEFAULT NOW()  -- Date the user registered
);
...
CREATE TABLE product_instance (
    ...
    CONSTRAINT fk_product_instance_product FOREIGN KEY (id_product) REFERENCES products(id_product) ON DELETE CASCADE,  -- Reference to the product
    CONSTRAINT fk_product_instance_user FOREIGN KEY (id_user) REFERENCES user_data(id_user) ON DELETE CASCADE  -- Reference to the user who owns this product instance
);
...
CREATE TABLE location_data (
    ...
    CONSTRAINT fk_location_product_instance FOREIGN KEY (id_product_instance) REFERENCES product_instance(id_product_instance) ON DELETE CASCADE  -- Link to product instance
);
...
CREATE TABLE esp32_data (
    ...
    CONSTRAINT fk_esp32_data_product_instance FOREIGN KEY (id_product_instance) REFERENCES product_instance(id_product_instance) ON DELETE CASCADE  -- Link to product instance
);
...
CREATE TABLE user_products (
    ...
    CONSTRAINT fk_user_products_user FOREIGN KEY (id_user) REFERENCES user_data(id_user) ON DELETE CASCADE,
    CONSTRAINT fk_user_products_product FOREIGN KEY (id_product) REFERENCES products(id_product) ON DELETE CASCADE
);
